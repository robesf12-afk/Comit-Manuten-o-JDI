...
/** Fontes para o onepagers.json (fallback automático) */
const ONEPAGERS_JSON_SOURCES = [
  "/banners_media/onepagers.json",
  // RAW do GitHub – ajuste o caminho/branch se mudar a branch:
  "https://raw.githubusercontent.com/robesf12-afk/Comit-Manuten-o-JDI/restore-layout-bom/public/banners_media/onepagers.json",
] as const;

/** Bases para resolver cada imagem (tenta local do preview e cai para RAW do GitHub) */
...
/** Tenta fazer GET de uma lista de URLs e devolve o primeiro JSON válido. */
...
/** Faz HEAD para checar se uma URL existe (200). */
...
/** Dado um conjunto de bases e um arquivo, devolve a primeira URL alcançável. */
async function resolveFirstReachable(bases: readonly string[], filename: string): Promise<string | null> {
  for (const base of bases) {
    // encodeURIComponent para lidar com espaços e acentos:
...
export default function App() {
  ...
  // Carrega o onepagers.json (preview -> RAW GitHub)
  useEffect(() => {
    (async () => {
      try {
  ...
  // Resolve URL de cada imagem (tenta local, se 404 usa RAW GitHub)
  useEffect(() => {
    (async () => {
      ...
      for (const name of onePagers) {
        ...
        else console.warn("[onepagers] não encontrei acessível:", name);
      }
      setOnePagerResolved(resolved);
    })();
  }, [onePagers]);
  ...
  return (
    <div className="app">
      ...
      <main className="content">
        ...
        {/* Loading / Nada para exibir */}
        {!err && !onePagersErr && onePagerSlides.length === 0 && (!cfg || (cfg.sections || []).length === 0) && (
          <div className="loading">Nada para exibir ainda. Verifique se os arquivos estão no preview desta branch.</div>
        )}
        ...
        {/* Fallback visual (caso o JSON remoto ainda não tenha carregado, mas há algo na página) */}
