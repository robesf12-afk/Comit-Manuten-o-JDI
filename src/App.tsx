...
/* ===== CTA de Notificações (compatível com initPush) ===== */
const NotifyCTA: React.FC = () => {
  const [show, setShow] = useState(false);
  const [perm, setPerm] = useState<NotificationPermission | "unknown">("unknown");
  const [isSupported, setIsSupported] = useState(true);

  // regra: mostrar se suporte ok e permissão não "granted"
  const decideShow = (p: NotificationPermission | "unknown", supported: boolean) => {
  ...
  useEffect(() => {
    ...
    const p = supported ? Notification.permission : "denied";
    setPerm(p);
    setShow(decideShow(p, supported));
  }, []);

  const request = async () => {
    try {
      // inicializa OneSignal (se ainda não)
      await initPush();

      // tenta pedir permissão via OneSignal v16
      try {
        // @ts-ignore
      ...
      setShow(decideShow(p as NotificationPermission, isSupported));
    }
  };
  ...
  return (
    <div className="notify-cta" role="region" aria-label="Ativar notificações">
      ...
      <div style={{ marginLeft: "auto", display: "flex", gap: 8 }}>
        <button className="notify-btn" onClick={request}>{denied ? "Como liberar" : "Ativar"}</button>
        <button className="notify-btn" style={{ background: "#777" }} onClick={() => setShow(false)} aria-label="Não mostrar de novo">
          Não mostrar
        </button>
      </div>
    </div>
...
export default function App() {
  ...
  /* Aviso iPhone só quando NÃO instalado */
  useEffect(() => {
    ...
    setShowIosBanner(isiOS && !standalone);
  }, []);
  ...
  /* Busca lista de OnePagers (sem autoplay) e ordena fixo */
  useEffect(() => {
    ...
    <div className="app">
      <style>{`
        ...
        @media (max-width:600px){
          ...
          .logo-comite{ grid-area:logo; height:32px; }
          .title-chip{ grid-area:title; }
          .logo-femsa{ grid-area:femsa; height:28px; }
        ...
        {bannerErro ? (
          ...
          <>
            <div className="banner-dinamico" onTouchStart={handleTouchStart} onTouchMove={handleTouchMove} onTouchEnd={handleTouchEnd}>
              {!isNarrow && onePagers.length > 1 && (
                <>
                  <button className="banner-arrow left" onClick={() => setBannerIndex((p) => (p - 1 + onePagers.length) % onePagers.length)} aria-label="Anterior">❮</button>
                  <button className="banner-arrow right" onClick={() => setBannerIndex((p) => (p + 1) % onePagers.length)} aria-label="Próximo">❯</button>
                </>
              )}
              <img src={encodeURI(currentOnePager)} alt={onePagers[bannerIndex]} />
            </div>
            {onePagers.length > 1 && (
