...
const SmartImg: React.FC<{
  ...
  onFinalError?: () => void; // ✅ NOVO: chamado quando todas as tentativas falham
    ...
    <img
      ...
      onError={(e) => {
        ...
        if (next < variants.length) {
          ...
          // ✅ NOVO: evita loading infinito quando a imagem nunca carrega
          onFinalError?.();
        }
      }}
    />
...
export default function App() {
  ...
  // ✅ NOVO: estados para não ficar "Carregando..." infinito quando JSON falha
  ...
  // ✅ assetV estável: evita corrida/troca de URL no meio do carregamento
  ...
  useEffect(() => {
    const loadQD = async () => {
      ...
      try {
        ...
        // ✅ ordena priorizando "atual" se existir, depois mantém o resto
              ...
              {onePagers.length > 1 && (
                <>
                  ...
                  <ArrowButton
                    side="right"
                    title="Próximo"
                    onClick={() => setBannerIndex((p) => (p + 1) % onePagers.length)}
                  />
                </>
              )}
          ...
          <>
            ...
            >
              {qdImgs.length > 1 && (
                <>
                  <ArrowButton
                    side="left"
                    title="Anterior"
                    onClick={() => setQdIndex((p) => (p - 1 + qdImgs.length) % qdImgs.length)}
                  />
              ...
              <SmartImg
                ...
                onFinalError={() => setReadyQD(true)} // ✅ destrava se falhar
              />
            </div>
          ...
          <>
            ...
            >
              {qdlImgs.length > 1 && (
                <>
                  <ArrowButton
                    side="left"
                    title="Anterior"
                    onClick={() => setQdlIndex((p) => (p - 1 + qdlImgs.length) % qdlImgs.length)}
                  />
                  <ArrowButton
                    side="right"
                    title="Próximo"
                    onClick={() => setQdlIndex((p) => (p + 1) % qdlImgs.length)}
                  />
                </>
              )}
              ...
              <SmartImg
                ...
                onFinalError={() => setReadyQDL(true)} // ✅ destrava se falhar
              />
            </div>
