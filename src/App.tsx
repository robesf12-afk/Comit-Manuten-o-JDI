...
/* =====================  CONFIG  ===================== */
/** Link público do seu config.json no OneDrive/SharePoint (mantido) */
const CONFIG_URL =
...
/** Caminhos para os One Pagers (local + fallback GitHub raw) */
const BRANCH = "restore-layout-bom";
const REPO_RAW_BASE =
  "https://raw.githubusercontent.com/robesf12-afk/Comit-Manuten-o-JDI/" +
  `${BRANCH}/public/banners_media/`;

const ONEPAGERS_JSON_LOCAL = "/banners_media/onepagers.json";
const ONEPAGERS_JSON_GITHUB =
  "https://raw.githubusercontent.com/robesf12-afk/Comit-Manuten-o-JDI/" +
  `${BRANCH}/public/banners_media/onepagers.json`;
...
/* =====================  TIPAGENS  ===================== */
type Slide = {
  img: string;          // URL da imagem principal (tenta local primeiro)
  fallbackImg?: string; // fallback (GitHub raw) se a principal falhar
...
export default function App() {
  ...
  // OnePagers (local + fallback GitHub)
  const [onePagers, setOnePagers] = useState<string[] | null>(null);
  ...
  // OnePagers.json (local -> fallback GitHub)
  useEffect(() => {
    (async () => {
      try {
        ...
        // 1) tenta local
        let res = await fetch(ONEPAGERS_JSON_LOCAL + `?v=${Date.now()}`, { cache: "no-store" });
        if (!res.ok) {
          // 2) fallback para GitHub raw
          res = await fetch(ONEPAGERS_JSON_GITHUB + `?v=${Date.now()}`, { cache: "no-store" });
        }
        if (!res.ok) throw new Error(`HTTP ${res.status}`);

        const list = (await res.json()) as unknown;
        if (!Array.isArray(list) || !list.every((x) => typeof x === "string")) {
          throw new Error("Formato inválido do onepagers.json");
        }
        setOnePagers(list as string[]);
      } catch (e) {
        console.error("Erro ao carregar onepagers.json:", e);
        setOnePagersErr("Não consegui carregar o onepagers.json.");
        setOnePagers([]); // evita ‘tela vazia’
      }
    })();
  }, []);

  // Mapeia para slides com fallback de imagem
  const onePagerSlides: Slide[] = useMemo(() => {
    const files = onePagers ?? [];
    return files.map((name) => {
      const encoded = encodeURIComponent(name); // preserva maiúsculas/minúsculas
      return {
        img: `/banners_media/${encoded}`,
        fallbackImg: REPO_RAW_BASE + encoded, // se local não estiver no preview, usa GitHub raw
      };
    });
  }, [onePagers]);
  ...
  return (
    <div className="app">
      ...
      <main className="content">
        ...
        {/* Seção ONE PAGER (independe do config.json) */}
        {onePagerSlides.length > 0 ? (
          ...
          !onePagersErr && <div className="loading">Carregando banners…</div>
        )}
