...
/* =====================  UTILS  ===================== */
function isImageUrl(url: string) { return IMG_RE.test(url); }
...
async function fetchJsonWithFallback<T = unknown>(urls: readonly string[]): Promise<T> {
  ...
  for (const u of urls) {
    try {
      ...
      console.warn("[onepagers] falhou:", u, e);
      lastErr = e;
    }
  }
...
class ErrorBoundary extends React.Component<{ children: React.ReactNode }, { error: any }> {
  ...
  render() {
    if (this.state.error) {
      return (
        <div style={{maxWidth:900, margin:"20px auto", padding:"14px 16px",
          borderRadius:12, border:"1px solid #f2c1c1", background:"#fff4f4", color:"#8a0f0f"}}>
          <strong>Opa, houve um erro ao renderizar a página.</strong>
...
const Carousel: React.FC<{ slides: Slide[]; height?: number | string; autoMs?: number }> = ({
  ...
  const content = useMemo(() => {
    if (isImageUrl(current.img)) {
      return <img src={current.img} alt="" style={{ width:"100%", height:"100%", objectFit:"contain", display:"block" }} />;
    }
    return (
      <div style={{ width:"100%", height:"100%", display:"grid", placeItems:"center", padding:16, textAlign:"center", color:"#333" }}>
        <div style={{ opacity:0.85, maxWidth:520 }}>
          ...
          <p style={{ margin:"10px 0 14px" }}>
            Este item é uma <strong>pasta/arquivo do SharePoint</strong>.<br/>Toque abaixo para abrir e visualizar.
          </p>
          <a href={current.link || current.img} target="_blank" rel="noopener noreferrer"
             style={{ display:"inline-flex", alignItems:"center", gap:8, padding:"10px 14px", borderRadius:10, background:"#cc0000",
                      color:"#fff", textDecoration:"none", boxShadow:"0 8px 20px rgba(0,0,0,.18)" }}>
            <IconLink /> Abrir no SharePoint
          </a>
        </div>
      </div>
    );
  ...
  return (
    <div className="carousel" style={{
      ...
      background:"#fff", boxShadow:"0 8px 24px rgba(0,0,0,.10)", overflow:"hidden"
    }}>
      {content}
      {len > 1 && (
        <>
          ...
          <div style={dotsWrapStyle}>
            {slides.map((_, i) => (
              <span key={i} onClick={() => setIdx(i)}
                style={{ width:8, height:8, borderRadius:999, background: i===idx ? "#cc0000" : "rgba(0,0,0,.25)", cursor:"pointer" }} />
            ))}
          </div>
        </>
      )}
...
const dotsWrapStyle: React.CSSProperties = { position:"absolute", bottom:10, left:0, right:0, margin:"0 auto", display:"flex", gap:8, justifyContent:"center", alignItems:"center" };
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        if (!data || !Array.isArray(data.sections)) throw new Error("Formato inválido do config.json");
        setCfg(data);
      } catch (e: any) {
        setErr("Não consegui ler o config.json do OneDrive. Verifique se o link é público e tente novamente.");
        console.error("Erro ao ler config.json:", e);
      }
    })();
  }, []);

  // ONE PAGERS (local -> RAW)
  useEffect(() => {
    (async () => {
      try {
  ...
  return (
    <ErrorBoundary>
      <div className="app">
        ...
        <aside className="drawer" role="dialog" aria-modal="true" aria-label="Categorias"
          ...
          <div className="drawer-header">
            ...
            <button onClick={() => setOpen(false)}
              ...
              aria-label="Fechar menu" title="Fechar">×</button>
          </div>
        ...
        <main className="content">
          ...
          {!cfg && !err && onePagerSlides.length > 0 && (
            <section className="section">
              ...
              <div className="card">
                <img src="/banner-reconhecimentos.png" alt="Reconhecimentos"
...
.menu-btn{ width:44px; height:44px; border:none; border-radius:999px; background:#b80000; color:#fff; box-shadow:0 4px 12px rgba(0,0,0,.25); display:grid; place-items:center; cursor:pointer; justify-self:start; margin-left:0; }
.menu-btn .bar{ width:22px; height:2px; background:#fff; margin:2.5px 0; border-radius:2px; display:block; }
.logo-comite{ height:46px; object-fit:contain; } .logo-femsa{ height:44px; object-fit:contain; justify-self:end; }
...
@media (max-width:430px){
  ...
  .menu-btn{ width:40px; height:40px; } .menu-btn .bar{ width:20px; } .logo-comite{ height:38px; } .logo-femsa{ height:34px; }
...
.drawer-overlay{ position:fixed; inset:0; background:rgba(0,0,0,.35); transition:opacity .2s; z-index:100; }
.drawer{ position:fixed; top:0; left:0; height:100dvh; width:320px; max-width:86vw; background:#fff; box-shadow:4px 0 24px rgba(0,0,0,.18); z-index:102; display:flex; flex-direction:column; transition:transform .22s; }
...
function useIsMobile() {
  const get = () => (typeof window !== "undefined" ? window.innerWidth <= 430 : false);
